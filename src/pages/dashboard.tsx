...
import { getMaintenances } from 'apiSdk/maintenances';
...
function DashboardPage() {
  ...
  const maintenanceStat = useSWR<PaginatedInterface<MaintenanceInterface>>(
    () => `/maintenances`,
    () => getMaintenances({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Maintenances',
      result: maintenanceStat?.data?.totalCount,
